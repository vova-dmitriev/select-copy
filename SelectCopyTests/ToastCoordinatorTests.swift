...
final class ToastCoordinatorTests: XCTestCase {
    func testUsesScreenContainingSelectionPointAndConfiguredPosition() {
        ...
        let screen = ToastScreen(frame: NSRect(x: 0, y: 0, width: 1000, height: 800), visibleFrame: NSRect(x: 0, y: 20, width: 1000, height: 760))
    ...
    func testDisabledToastDoesNotShowPanel() {
        let settings = SettingsStore(userDefaults: UserDefaults(suiteName: UUID().uuidString)!)
    ...
    func testEmptyCustomTextUsesIconOnlyContent() {
        let settings = SettingsStore(userDefaults: UserDefaults(suiteName: UUID().uuidString)!)
        ...
        let screen = ToastScreen(frame: NSRect(x: 0, y: 0, width: 500, height: 500), visibleFrame: NSRect(x: 0, y: 0, width: 500, height: 500))
        let coordinator = ToastCoordinator(
            settings: settings,
            localizer: Localizer(language: .english),
            panel: panel,
...
private final class ToastPanelSpy: ToastPaneling {
    ...
    init(contentSize: NSSize) { self.contentSize = contentSize }
...
private final class ToastScreensSpy: ToastScreenProviding {
    ...
    init(screens: [ToastScreen]) { self.screens = screens }
    func screen(containing point: CGPoint) -> ToastScreen? { screens.first { $0.frame.contains(point) } }
    var mainScreen: ToastScreen? { screens.first }
}
